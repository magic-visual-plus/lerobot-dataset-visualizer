...
function EpisodeViewerInner({
  ...
  useEffect(() => {
    if (!isLoading) {
      console.log(
        `[perf] Loading complete in ${(performance.now() - loadStartRef.current).toFixed(0)}ms`,
      );
    }
      ...
      if (key === " ") {
        ...
        if (activeTab === "urdf") {
          ...
          if (
            ...
            navigate(`/${org}/${dataset}/episode_${nextEpisodeId}`);
          }
        }
      }
    },
    [activeTab, episodeId, episodes, navigate, setIsPlaying, urdfEpisode, org, dataset],
  );
